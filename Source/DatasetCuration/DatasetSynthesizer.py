...
class rec_model:
    ...
    def predict_float(self, img):
        ...
        # Get top k character predictions
        k = 3
        #print(preds.shape) -> 40,1,90
        for i in range(preds.shape[0]):
            data = preds[i]
            print(data.shape)
            top_k_indices = np.argsort(data, axis=-1)[:, -k:]
            top_k_indices = np.flip(top_k_indices, axis=-1)
            print(f'Top {k} indices')
            print(top_k_indices)
            top_k_scores = np.take_along_axis(data, top_k_indices, axis=-1)
            print(f'Top {k} probabilities')
            print(top_k_scores)

        # Get the character confidences
        confidences = np.max(preds, axis=2)
        # Get the indexes for each character which has the max. probability
        preds = np.argmax(preds, axis=2)
        # Calculate average of confidences as the sentence confidence
        avg_confidences = np.mean(confidences)
        print(f'Avg Sentence Confidence: {avg_confidences}')
...
def init_args():
    ...
    parser.add_argument('--image_path', default='data/images/', type=str, help='img path for predict')
    parser.add_argument('--output_folder', default='output/predict.jpg', type=str, help='img path for output')
    parser.add_argument('--dataset_raw', default='output/dataset_raw/', type=str, help='img path for raw dataset imgs')
...
def synthesize(input_path, output_path):
    ...
    for path in files_list:
        try:
            ...
            _, boxes_list = detection_model.predict(img, path)
